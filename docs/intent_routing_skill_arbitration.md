...
| App 内搜索 | `打开淘宝搜索耳机`、`在当前应用搜索耳机` | Static app-search skill；有已校验 `mobile-action-270m` 时走 model-driven bootstrap | V1 只做低风险搜索闭环；observation 只给本地动作模型，不暴露给远端；不发送、删除、支付、下单、授权或发布。 |
...
- App 搜索：默认使用静态 Skill；当本地 action model 已校验时，Skill 只负责
