...
- 分享二进制、图片、音频、视频、PDF 或 Office 附件时，App 只应读取 MIME 类型、文件名和大小等元数据，不应读取文件正文或二进制内容。
...
- 当前只验收分享入口、语音入口、`text/*` 有界摘录与 metadata-only 边界；截图捕获、相册选择、完整文档解析、OCR、Office/PDF 解析和媒体内容理解仍是待实现项。
