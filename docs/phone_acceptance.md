...
真实远程模型检查使用 debug APK 和 `scripts/live_remote_emulator.sh`。脚本只从
`POCKETMIND_LIVE_REMOTE_API_KEY` 或 `DEEPSEEK_API_KEY` 读取密钥，默认 endpoint 为
...
- 低风险只读 `web_search` 不应打开浏览器或展示确认卡；例如“北京天气怎么样”应直接调用 Web 搜索工具并在聊天中组织结果。
