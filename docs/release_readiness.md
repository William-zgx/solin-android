...
| Contract hardening required | Release operations owner | Strengthen monitoring and rollback evidence beyond path/SHA checks: require passed typed properties evidence, stable targets, and binding to the operations record fields. | `scripts/verify_release_operations_record.sh`; add negative tests with matching SHA but pending or weak monitoring/rollback evidence. |
| Contract hardening required | Local AI / validation owner | Add local-vision release-flow evidence alongside the existing remote-vision flow: local runtime image attachment sent, LocalOnly persistence, prompt metadata redaction, remote runtime idle, and unsupported OCR skip behavior. | `scripts/record_release_flow_evidence.sh`, `scripts/collect_release_flow_matrix_evidence.sh`, and `scripts/verify_release_validation_record.sh`. |
