...
推荐使用 Hugging Face LiteRT Community 的 Gemma 4 E2B LiteRT-LM 模型：
...
文件名是 `gemma-4-E2B-it.litertlm`，大小约 2.59 GB，建议使用 Wi-Fi，并确保手机有足够空间。下载或导入成功后会自动加载；GPU 初始化失败时会自动切到 CPU。
...
本地单元测试覆盖模型文件名、精确大小、存储空间、下载状态、进度和字节显示。连接真机后可再跑启动冒烟测试：
...
1. 首次启动看到模型准备页，能打开模型来源。
2. Wi-Fi 下点击“下载推荐模型”，进度可见，完成后自动加载。
3. 断网后可以继续问答。
4. 导入一个已有 `.litertlm` 文件时显示复制进度，并能加载。
5. 如果 GPU 初始化失败，App 自动切到 CPU 并仍可问答。
