...
object CapabilityMatrix {
    const val productPositioning: String =
        "隐私优先的随身 AI 助手：本地对话优先留在本机，已校验且支持视觉的本地模型可在本机处理图片；远程模型需配置并切换后使用；手机操作默认保守确认，低风险连续操作可按设置减少弹窗。"
    const val targetUserJob: String =
        "在手机上处理私人日常问答、记忆、需视觉支持的图片输入、用户主动提供的文件摘录、App 打开和低风险搜索交互，同时默认把本地上下文留在本机。"
    ...
    val sensitiveCapabilityDisclosures: List<SensitiveCapabilityDisclosure> =
        listOf(
            SensitiveCapabilityDisclosure(
                ...
                consentBoundary = "配置并切换远程模型后，普通文本按提醒策略展示发送预览；图片和疑似敏感内容逐次确认，确认前不请求远程接口。",
                remoteBoundary = "远程请求只发往用户配置的 HTTPS 或本机调试 endpoint；图片字节只有在主动附加、配置显式启用图片输入、接口支持 OpenAI image_url 视觉输入且用户确认后发送。",
            ...
            SensitiveCapabilityDisclosure(
                ...
                displayName = "分享和文件选择",
                dataAccessed = "用户通过分享入口或系统文件选择器主动提供的文本、文件元数据、受限文本摘录，以及已校验且支持视觉的本地模型可使用的受限图片字节。",
                consentBoundary = "仅处理用户主动分享或通过系统文件选择器确认的内容；本地模式才读取受支持文本/PDF/OCR 摘录；已校验且支持视觉的本地模型可在发送时读取受限图片字节，不支持视觉时不自动 OCR。",
                remoteBoundary = "远程模式保护分享文本、非图片附件、文本摘录和 OCR 摘录；图片只有在已配置并切换远程模型、视觉开启、接口支持 OpenAI image_url 且用户确认发送后才走远程视觉路径；本地视觉图片字节不进入远程请求。",
