...
class LegacyPrefsMigrator(
    ...
    private fun migrateSettings(prefs: android.content.SharedPreferences) {
        ...
        runCatching {
            settingsStore.saveInferenceMode(
                com.bytedance.zgx.pocketmind.InferenceMode.valueOf(prefs.getString("inference_mode", "").orEmpty()),
            )
        }
        runCatching {
            settingsStore.saveBackend(
                com.bytedance.zgx.pocketmind.BackendChoice.valueOf(prefs.getString("backend", "").orEmpty()),
            )
        }
    }
    ...
    private fun migrateRemote(prefs: android.content.SharedPreferences) {
        settingsStore.saveRemoteConfig(
            com.bytedance.zgx.pocketmind.RemoteModelConfig(
                ...
                add(
                    LegacyMessage(
                        ...
                        tokensPerSecond = stats?.optDouble("tokensPerSecond")?.takeIf { value ->
                            com.bytedance.zgx.pocketmind.GenerationStats(1, value).isUsable()
                        },
                    ),
                )
            }
    ...
    private data class LegacyMessage(
        ...
        fun toChatMessage(): com.bytedance.zgx.pocketmind.ChatMessage =
